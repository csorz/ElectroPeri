...
const DEFAULT_CODE = `// Go 在线运行工具
// 注意：此工具需要后端支持才能实际运行 Go 代码
...
export default function GoRunnerToolPage() {
  ...
  const runCode = () => {
    ...
    // 由于浏览器无法直接运行 Go，显示提示信息
    setError('Go 代码需要后端服务支持才能运行。当前为演示模式。')
    setOutput(`提示：
1. 在浏览器中无法直接运行 Go 代码
2. 需要部署后端服务（如使用 Go 编译器）
3. 可考虑以下方案：
